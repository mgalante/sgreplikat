define(['plugins/http','helpers/serializer','jquery','knockout', 'helpers/appSettings'],function(http,serializer,$,ko,appSettings){
    ...
    var productos = {
        getAll: function()
        {
            ...
            http.get(appSettings.root + "productos/list").then(function(data){
        ...
        {
            var dfd = new $.Deferred();
            http.get(appSettings.root + "productos/get/" + id).then(function(data){
                dfd.resolve(serializer.deserialize(JSON.stringify(data)));
            }).fail(function(){
                dfd.reject();
            });
        ...
        save: function(item)
        {
            return $.ajax({
                dataType: 'json',
                type: 'post',
                url: appSettings.root + "productos/save",
        ...
        delete: function(id)
        {
            return $.ajax({
                dataType: 'json',
                type: 'post',
                url: appSettings.root + "productos/delete/" + id
            });
        }
    };
    return productos;
